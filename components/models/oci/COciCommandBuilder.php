	...
	public function createInsertCommand($table,$data)
	{
		...
		foreach($data as $name=>$value)
		{
			if(($column=$table->getColumn($name))!==null && ($value!==null || $column->allowNull))
			{
				...
				{
					$tcvalue = $column->typecast($value);
					if($tcvalue instanceof CDbExpression)
					{
						$placeholders[]=$tcvalue->expression;
						foreach($tcvalue->params as $n=>$v)
							$values[$n]=$v;
					} else {
						$placeholders[]=self::PARAM_PREFIX.$i;
						$values[self::PARAM_PREFIX.$i] = $tcvalue;
						$i++;
					}
				}
			}

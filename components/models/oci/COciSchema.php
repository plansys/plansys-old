...
EOD;
    ...
    protected function findTableNames($schema = '') {
        if ($schema === '') {
            $sql     = <<<EOD
SELECT table_name as table_name , '{$schema}' as table_schema FROM user_tables 
UNION
SELECT view_name as table_name, '{$schema}' as table_schema from user_views
EOD;
            $command = $this->getDbConnection()->createCommand($sql);
        } else {
            $sql     = <<<EOD
SELECT object_name as table_name, owner as table_schema FROM all_objects
WHERE (object_type = 'TABLE' OR object_type = 'VIEW')  AND owner=:schema
EOD;
            $command = $this->getDbConnection()->createCommand($sql);

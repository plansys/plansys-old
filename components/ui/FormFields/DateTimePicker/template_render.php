    ...
    <div class="<?= $this->fieldColClass ?>" >
        ...
        <div ng-if="['monthyear', 'date'].indexOf(fieldType) >= 0" style="padding-top:5px;">
            <select ng-disabled="isDPDisabled" ng-options="item for item in dayList" ng-show="fieldType == 'date'"
                    ...
                    ng-model="dd.day" ng-change="changeDropdown()"></select>&nbsp;<select ng-disabled="isDPDisabled" ng-options="item.i as item.n for item in monthList"
                    name="<?= $this->renderName ?>[month]" id="<?= $this->renderID ?>_month"
                    ng-model="dd.month" ng-change="changeDropdown()"></select>&nbsp;<select ng-disabled="isDPDisabled" ng-options="item for item in yearList"
